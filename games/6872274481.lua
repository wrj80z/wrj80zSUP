		...
		Normal = function(ent)
			...
			if nametag then
				...
				if Equipment.Enabled and store.inventories[ent.Player] then
					...
					nametag.Kit.Image = render
				end

				if ShowRank.Enabled then
					bedwars.Client:Get("FetchRanks"):CallServerAsync({ent.Player.UserId}):andThen(function(self)
						self = self[1]
						local division = -1
					...
					if sword then
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								...
								local num = ClosetMode.Enabled and 13 or AttackRange.Value 
									...
									if MutiAura.Enabled and not ClosetMode.Enabled then
										if AirHit.Enabled then
											...
											if (state == Enum.HumanoidStateType.Jumping or state == Enum.HumanoidStateType.Freefall)
												and chance > AirHitsChance.Value then
												canHitTarget = false
											end
										end
...
run(function()
		...
		if BackTrackIncoming.Enabled then
			...
			elseif Types.Value == "LagBased" then
				...
				if os.time() - LastTP < 2 then
					finalLag = finalLag * (6 + 9 * 1.8)
				else
					finalLag = finalLag * (1 + 3 * 0.8)
				end
			elseif Types.Value == "Manual" then
				finalLag = finalLag - ((math.random() + Random.new():NextNumber(2,4)))
			elseif Types.Value == "Power" then
				finalLag = finalLag * ((math.random() + Random.new():NextNumber(2,4)))
				local velocity = entitylib.character.HumanoidRootPart.Velocity.Magnitude
				if velocity > 10 then
					finalLag = finalLag * (3 + 6 * 1.5)
				end
					
				local lastDamage = entitylib.character.Character:GetAttribute('LastDamageTakenTime') or 0
				if tick() - lastDamage < 2 then
					finalLag = finalLag * (3 + 6 * 1.7)
				else
					finalLag = finalLag * (6 + 12 * 2.3)
				end
				local LastTP = lplr:GetAttribute('LastTeleport') or (os.time() - 2)
				local dec = 1 + (math.random() + Random.new():NextNumber(6,8))
				LastTP = LastTP + dec
				if os.time() - LastTP < 2 then
					finalLag = finalLag * (9 + 12 * 2.8)
				else
					finalLag = finalLag * (3 + 6 * 1.8)
				end
				finalLag = finalLag / (((math.random() - math.random() * math.random()) + Random.new():NextNumber(4,6)))
			end
	...
	Types = BackTrack:CreateDropdown({
		Name = "Types",
		List = {'LagBased','Dynamic','Manual','Power'},
		Default = 'Manual',
				Visible = true
	})
	...
	local function KitColl(id, func, range)
		...
		repeat
			if entitylib.isAlive then
				local localPosition = entitylib.character.RootPart.Position
				for _, v in objs do
					if InfiniteFly.Enabled or not AutoTal.Enabled then break end
					local part = not v:IsA('Model') and v or v.PrimaryPart
					if part and (part.Position - localPosition).Magnitude <= (range) then
						func(v)
					end
...
run(function()
			...
			if callback then
				...
				repeat
					...
					if sword then
						...
						local plrs = resolveAttackTargets(nonplrs)
						local anyInSwingRange = false
						if ASOPT.Enabled and LastAuraTarget and not AfterSwingDone then
							local lastTargetAlive = LastAuraTarget.Character and LastAuraTarget.Character.PrimaryPart
							if lastTargetAlive then
								local distToLast = (LastAuraTarget.Character.PrimaryPart.Position - selfpos).Magnitude
								if distToLast <= SwingRange.Value then
									anyInSwingRange = true
								end
							end
						end
						if #plrs > 0 then
							...
							for _, v in plrs do
								...
								if SyncHit.Enabled then
									...
									local lastHit = lastHitTimes[v] or 0
									local timeSince = tick() - lastHit
									local requiredDelay = math.max(swingSpeed * 0.8 / math.max(#plrs, 1), 0.05)
									if timeSince < requiredDelay then
										continue
									end
								end
							...
							if LastAuraTarget and not AfterSwingDone and ASOPT.Enabled and meta and not anyInSwingRange then
								AfterSwingDone = true
								task.spawn(function()
									for i = 1, ASMS.GetRandomValue() do
										if not Killaura.Enabled then break end
										if not entitylib.isAlive then break end
										local targetRoot = LastAuraTarget and LastAuraTarget.Character and LastAuraTarget.Character.PrimaryPart
										if targetRoot then
											local currentDist = (targetRoot.Position - entitylib.character.RootPart.Position).Magnitude
											if currentDist <= SwingRange.Value then
												break
											end
										end
										bedwars.SwordController:playSwordEffect(meta, false)
					...
					-- FIX: Properly enforce ChargeTime as the swing interval
					if #attacked > 0 then
						local elapsed = tick() - CurrentSwingTICK
						local remaining = ChargeTime.Value - elapsed
						if remaining > 0.01 then
							...
							task.wait(math.max(1 / UpdateRate.Value - tme, 0))
						end
					else
						local tme = 0
						if SyncHit.Enabled then
	...
	ASMS = Killaura:CreateTwoSlider({
		...
		DefaultMin = 4,
		DefaultMax = 10,
		Tooltip = 'keeps swinging based X amount of times after target leaves swing range',
		Darker = true,
		Visible = false
	})
	ASOPT = Killaura:CreateToggle({
		...
		Default = false,
		Visible = true,
		Tooltip = 'enables the after swing feature (only fires when target leaves swing range)',
		Function = function(v)
			ASMS.Object.Visible = v
		end
	})
