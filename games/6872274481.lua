...
local cyanpred = oldpred
local synpred = oldpred
local zikopred = oldpred
...
local function switchItemV2(tool, delayTime)
	...
	task.delay(delayTime,function()
		bedwars.Client:Get(remotes.ItemEquip):CallServer({hand = tool})
	end)
end
...
function PR:InvokeServer(tool,meta,pos,dir,CLP)
	...
	if tool and meta and pos and dir then
		...
		if hasArrow then
            ...
            local PF = bedwars.Client:WaitFor(remotes.FireProjectile)
            local function cb(event)
				...
				if CLP then
					bedwars.ProjectileController:createLocalProjectile(meta, tool, arrowName, pos, ID1, dir, {drawDurationSeconds = meta.drawDurationSeconds})
               		event:CallServerAsync(tool,arrowName,tostring(meta),pos,dir,ID2,{shotId=ID1,drawDurationSec=meta.drawDurationSeconds},workspace:GetServerTimeNow() - 0.045)
				else
                	event:CallServerAsync(tool,arrowName,tostring(meta),pos,dir,ID2,{shotId=ID1,drawDurationSec=meta.drawDurationSeconds},workspace:GetServerTimeNow() - 0.045)
				end
            end
            ...
            local function cb(event)
				...
				if CLP then
					...
					event:CallServerAsync(tool,tostring(meta),pos,dir,ID2,{shotId=ID1,drawDurationSec=meta.drawDurationSeconds},workspace:GetServerTimeNow() - 0.045)
				else
                	event:CallServerAsync(tool,tostring(meta),pos,dir,ID2,{shotId=ID1,drawDurationSec=meta.drawDurationSeconds},workspace:GetServerTimeNow() - 0.045)
				end
            end
...
function WhisperController:request(type,target)
	if type == "Heal" then
		repeat task.wait(0.1) until bedwars.AbilityController:canUseAbility('OWL_HEAL')
	...
	elseif type == "Fly" then
		repeat task.wait(0.1) until bedwars.AbilityController:canUseAbility('OWL_LIFT')
	...
	elseif type == "Unmount" then
		repeat task.wait(0.1) until bedwars.AbilityController:canUseAbility('DEACTIVE_OWL')
		if bedwars.AbilityController:canUseAbility('DEACTIVE_OWL') then
			bedwars.Client:Get(remotes.RemoveOWL):SendToServer()
			task.wait(0.05)
			bedwars.AbilityController:useAbility('DEACTIVE_OWL')
		end
	elseif type == "Mount" then
		repeat task.wait(0.1) until bedwars.AbilityController:canUseAbility('SUMMON_OWL')
		if bedwars.AbilityController:canUseAbility('SUMMON_OWL') then
			bedwars.Client:Get(remotes.SummonOWL):CallServer(target)
			task.wait(0.05)
...
function NazarController:request(type)
	if type == "enabled" then
		repeat task.wait(0.1) until bedwars.AbilityController:canUseAbility('enable_life_force_attack')
	...
	elseif type == "heal" then
		repeat task.wait(0.1) until bedwars.AbilityController:canUseAbility('consume_life_foce')
...
run(function()
    ...
    local shakeTime = 0
    local hasMouseMove = typeof(mousemoveabs) == "function"
    ...
    local function optionAllowsAim()
        ...
        if opt == "First Person Only" then return isFirstPerson() end
        if opt == "Third Person Only" then return not isFirstPerson() end
        return true 
    end
    ...
    local function getTargetEntity()
        ...
        if Priorty.Enabled and priorityTarget then
            local pr = priorityTarget
            local hum = pr.Character and pr.Character:FindFirstChild("Humanoid")
            local alive = hum and hum.Health > 0
            local inRange = pr.RootPart and (pr.RootPart.Position - root.Position).Magnitude <= Distance.Value
            if alive and inRange then
                return pr
            else
                priorityTarget = nil
            end
		...
		local ent = not KillauraTarget.Enabled
    ...
    AimAssist = vape.Categories.Combat:CreateModule({
        ...
        Function = function(callback)
            ...
            AimAssist:Clean(runService.Heartbeat:Connect(function(dt)
                if GUI.Enabled and bedwars.AppController:isLayerOpen(bedwars.UILayers.MAIN) then return end
                shakeTime += dt
                if not entitylib.isAlive then return end
                if Limits.Enabled and store.hand.toolType ~= "sword" then return end
                ...
                if Shake.Enabled then
                    ...
                    local power  = ShakeV.Value * 0.6
                    shakeOffset = gameCamera.CFrame.RightVector * (wave * power) + gameCamera.CFrame.UpVector * (jitter * power * 0.5)
                end
                ...
                if TweenOption.Enabled then
                    ...
                    alpha = math.clamp(easeFunc(Smoothness.Value * dt), 0, 1)
                else
                    local strafeBonus = StrafeIncrease.Enabled and (inputService:IsKeyDown(Enum.KeyCode.A) or inputService:IsKeyDown(Enum.KeyCode.D)) and 10 or 0
                    alpha = math.clamp((AimSpeed.Value + strafeBonus) * dt * Smoothness.Value, 0, 1)
                end
    ...
    Priorty = AimAssist:CreateToggle({
        Name = "Priority",
        Tooltip = "Locks onto the first target until it becomes invalid",
    })
    ...
    Sort = AimAssist:CreateDropdown({Name = "Target Mode", List = methods})
    ...
    Shake = AimAssist:CreateToggle({
        ...
        Default = false,
        Visible = true,
        Function = function(v) ShakeV.Object.Visible = v end,
    })
    ...
    Smoothness = AimAssist:CreateSlider({
        Name = "Smoothness",
        Tooltip = "Set to 1 for original AA feel",
        Min = 1, 
		Max = 12, 
		Default = 1,
        Visible = true,
    ...
    TweenOption = AimAssist:CreateToggle({
        ...
        Function = function(v) TweenStyles.Object.Visible = v end,
    })
end)
	...
	local function start()
		...
		Thread = task.delay(1 / (store.hand.toolType == 'block' and BlockCPS or CPS).GetRandomValue(), function()
			repeat
				...
				local isBlock = toolType == 'block'

				if not bedwars.AppController:isLayerOpen(bedwars.UILayers.MAIN) then
					if isBlock and Block.Enabled then
						...
						if blockPlacer then
							if canDebug then
								if inputService.TouchEnabled then
									...
									if (now - lastPlace) >= (1 / 12 * 0.5) then
				...
				task.wait(1 / (isBlock and BlockCPS or CPS).GetRandomValue())
			until not AutoClicker.Enabled or not isClicking
		end)
	end
		...
		Function = function(callback)
			if callback then
				...
				AutoClicker:Clean(inputService.InputEnded:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 and Thread and (os.clock() - swapping) > 0.12 then
						stop()
					end
				end))
				...
				if inputService.TouchEnabled then
					pcall(function()
						for _, v in {'2', '5'} do
							AutoClicker:Clean(lplr.PlayerGui.MobileUI[v].MouseButton1Down:Connect(start))
							AutoClicker:Clean(lplr.PlayerGui.MobileUI[v].MouseButton1Up:Connect(stop))
						end
					end)
				end
			else
...
run(function()
	...
	local oldmine
	local oldplacement
	local CombatConstant = bedwars.CombatConstant
	local BlockBreakController = bedwars.BlockBreakController
	local BlockPlacementController = bedwars.BlockPlacementController
	local BlockEngine = bedwars.BlockEngine
	...
	Reach = vape.Categories.Combat:CreateModule({
		...
		Function = function(callback)
			if callback then
				oldreach = CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE or 14.4
				local blockBreaker = BlockBreakController:getBlockBreaker()
				if blockBreaker then
					oldmine = blockBreaker:getRange() or 14.4
				end
				oldplacement = BlockEngine.isAllowedPlacement
				CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE = Attack.Value + 2
				if blockBreaker then
					blockBreaker:setRange(Mine.Value)
				end
				BlockEngine.isAllowedPlacement = function(self, player, blockType, position, rotation, mouseBlockInfo)
					local result = oldplacement(self, player, blockType, position, rotation, mouseBlockInfo)
					if not result and player == LocalPlayer then
				...
				task.spawn(function()
					repeat task.wait(0.1) until BlockPlacementController or not Reach.Enabled
				...
				task.spawn(function()
					...
					local lastMineCheck = 0
					local lastPlaceCheck = 0
					local currentTime
					
					while Reach.Enabled do
						currentTime = os.clock()
						if currentTime - lastAttackCheck >= 1 then
							if CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE ~= Attack.Value + 2 then
								CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE = Attack.Value + 2
							end
							lastAttackCheck = currentTime
						end
						if currentTime - lastMineCheck >= 1 then
							pcall(function()
								local blockBreaker = BlockBreakController:getBlockBreaker()
							...
							lastMineCheck = currentTime
						end
						if currentTime - lastPlaceCheck >= 1 then
							pcall(function()
								local blockPlacer = BlockPlacementController:getBlockPlacer()
								if blockPlacer and blockPlacer.blockHighlighter then
				...
				CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE = oldreach or 14.4
				pcall(function()
					local blockBreaker = BlockBreakController:getBlockBreaker()
					if blockBreaker then
						blockBreaker:setRange(oldmine or 18)
					end
				end)
				...
				if oldplacement then
					BlockEngine.isAllowedPlacement = oldplacement
				end
				oldreach, oldmine, oldplacement = nil, nil, nil
			end
		end,
						...
						if check then
							...
							if Mode.Value == 'Lag' then
								...
								if chance < Chance.Value then
									...
									task.delay(Ground:GetRandomValue() / 1000 + 0.1, function()
										if air == true then
											air = false
											task.wait(0.05)
										end
		...
		Normal = function(ent)
			...
			task.spawn(function()
				if Rank.Enabled and ent.Player then
					...
					bedwars.Client:Get(remotes.Ranks):CallServerAsync({plr.UserId}):andThen(function(data)
						data = data[1]
						local division = data.rankDivision
						if division == nil then
							image = ''
						else
							image = bedwars.RankMeta[division].image
						end
					end)
					Icon.Image = image
					Icon.Parent = nametag
			...
			task.spawn(function()
				if Enchants.Enabled and ent.Player then
					...
					Icon.Size = UDim2.fromOffset(30, 30)
					Icon.Position =  UDim2.fromOffset(size.X + 18, -4)
					Icon.BackgroundTransparency = 1
					Icon.Image = ''
					Icon.Parent = nametag
				end
	...
	NameTags = vape.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				task.spawn(function()
					if Enchants.Enabled then
						bedwars.Client:OnEvent(remotes.ResearchEnchant,function(data)
							if not Enchants.Enabled then
								return
							end
							for _, ent in entitylib.List do
								local plr = playersService:GetPlayerFromCharacter(data.entityInstance)
								local tnt = playersService:GetPlayerFromCharacter(ent.Character)
								if plr == tnt then
									local enchant = data.enchant
									local nametag = Reference[ent]
									local meta = require(replicatedStorage.TS.enchant["enchant-meta"])
									nametag.EnchantIcon.Image = meta[enchant].image
								end
							end
						end)
					end
...
run(function()
							...
							if Open.Enabled then
								...
								if nearestChest then
									...
									if nearestChest ~= currentChest then
										...
										if TeamFilter.Enabled then
											local ChestTeam = nearestChest:GetAttribute('Team') or 0
											local MyTeam = lplr.Character:GetAttribute("Team") or -1
											if chestTeam == myTeam then
												return
											end
										end			
	...
	TeamFilter = ChestSteal:CreateToggle({Name='Team Filter'})
...
run(function()
	...
	local function buyItem(item, currencytable)
		...
		bedwars.Client:Get(bedwars.PurchaseItem):CallServerAsync({
			shopItem = item,
			shopId = id
		...
		Function = function(callback)
			if callback then
	            BedBreakEffect:Clean(vapeEvents.BedwarsBedBreak.Event:Connect(function(data)
	                firesignal(bedwars.Client:Get(remotes.BedBreak).instance.OnClientEvent, {
	                    player = data.player,
...
run(function()
	...
	local TexturesRemovedProcessedObjects = {}	
	...
	local function RemoveOrbs(obj)
		if OrbTargets[obj.Name] then
			table.insert(cachedObjects,{Parent=OrbTargets.Parent,ID=httpService:GenerateGUID(true),SpawnedTime=time(),o=obj})
			OrbTargets.Parent = replicatedStorage:WaitForChild('OnyxFPSBoostFolder',9e9)
		end
	end

	local function PlaceOrbsOGSlot()
		for i, v in cachedObjects do
			if v.o then
				if v.Parent then
					if v.SpawnedTime < time() then
						if #v.ID == 38 then
							v.o.Parent = v.Parent
						end
					end
	...
	local function removeTextures(obj)
		if isRestoring or TexturesRemovedProcessedObjects[obj] then return end
		
		if obj.Name == "Nametag" and obj.Parent.Name == "Head" and obj.Parent.Parent:FindFirstChild("Humanoid") then
			return
		end
		...
		if obj:IsA("BasePart") then
			...
			obj.Material = TextureMaterials[MaterialSelector.Value] or Enum.Material.SmoothPlastic
			
			if obj:IsA("MeshPart") then
				obj.TextureID = ""
			end
			
			TexturesRemovedProcessedObjects[obj] = true
	...
	local function cleanupDestroyedObjects()
		for obj, _ in pairs(originalTextures) do
			if not obj or not obj.Parent then
				...
				TexturesRemovedProcessedObjects[obj] = nil
			end
		end
	end
	...
	local function RamSaverProcess(db)
		if db then
			local Lighting = cloneref(game:GetService("Lighting"))
			local Terrain = cloneref(workspace:FindFirstChildOfClass("Terrain"))
			pcall(function()
				RamSaverSets.SavedQualityLevel = UserSettings():GetService("UserGameSettings").SavedQualityLevel
				UserSettings():GetService("UserGameSettings").SavedQualityLevel = Enum.SavedQualitySetting.QualityLevel1
			end)

			pcall(function()
				RamSaverSets.RenderingLevel = settings().Rendering.QualityLevel1
				settings().Rendering.QualityLevel = Enum.QualityLevel.Level01
			end)

			task.spawn(function()
				table.insert(RamSaverSets.LightingCachedSets,{Name='GlobalShadows',OG=Lighting.GlobalShadows,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				table.insert(RamSaverSets.LightingCachedSets,{Name='FogEnd',OG=Lighting.FogEnd,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				table.insert(RamSaverSets.LightingCachedSets,{Name='Brightness',OG=Lighting.Brightness,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				table.insert(RamSaverSets.LightingCachedSets,{Name='EnvironmentDiffuseScale',OG=Lighting.EnvironmentDiffuseScale,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				table.insert(RamSaverSets.LightingCachedSets,{Name='EnvironmentSpecularScale',OG=Lighting.EnvironmentSpecularScale,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				table.insert(RamSaverSets.LightingCachedSets,{Name='OutdoorAmbient',OG=Lighting.OutdoorAmbient,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
			end)
			...
			for _, v in ipairs(Lighting:GetChildren()) do
				if v:IsA("PostEffect") then
					table.insert(RamSaverSets.LightingCachedSets,{Name=v.Name,OG=v.Enabled,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
					v.Enabled = false
				end
			end
			...
			if Terrain then
				task.spawn(function()
					table.insert(RamSaverSets.TerrainCachedSets,{Name='WaterWaveSize',OG=Terrain.WaterWaveSize,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
					table.insert(RamSaverSets.TerrainCachedSets,{Name='WaterWaveSpeed',OG=Terrain.WaterWaveSpeed,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
					table.insert(RamSaverSets.TerrainCachedSets,{Name='WaterReflectance',OG=Terrain.WaterReflectance,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
					table.insert(RamSaverSets.TerrainCachedSets,{Name='WaterTransparency',OG=Terrain.WaterTransparency,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				end)
			...
			local function isInGui(inst)
				return inst:FindFirstAncestorOfClass("PlayerGui") ~= nil
			end
			local function optimizeInstance(v)
				if isInGui(v) then return end

				if v:IsA("ParticleEmitter") or v:IsA("Trail") or v:IsA("Beam") then
					v.Enabled = false
					table.insert(RamSaverSets.PartsDetailCachedSets,{Name=v.Name,OG=v.Enabled,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				elseif v:IsA("Explosion") then
					v.Visible = false
					table.insert(RamSaverSets.PartsDetailCachedSets,{Name=v.Name,OG=v.Visible,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				elseif v:IsA("Fire") or v:IsA("Smoke") or v:IsA("Sparkles") then
					v.Enabled = false
					table.insert(RamSaverSets.PartsDetailCachedSets,{Name=v.Name,OG=v.Enabled,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				elseif v:IsA("Decal") or v:IsA("Texture") then
					v.Parent = replicatedStorage
					table.insert(RamSaverSets.PartsDetailCachedSets,{Name=v.Name,OG=v,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				...
				elseif v:IsA("MeshPart") then
					...
					table.insert(RamSaverSets.PartsDetailCachedSets,{Name=v.Name,OG=v.TextureID,OG2=v.Material,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				elseif v:IsA("SpecialMesh") then
					v.TextureId = ""
					table.insert(RamSaverSets.PartsDetailCachedSets,{Name=v.Name,OG=v.TextureId,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				...
				elseif v:IsA("BasePart") then
					...
					v.Reflectance = 0
					v.Material = Enum.Material.Plastic
					table.insert(RamSaverSets.PartsDetailCachedSets,{Name=v.Name,OG=v.CastShadow,OG2=v.Material,OG3=v.Reflectance,SpawnedTime=time(),ID=httpService:GenerateGUID(true)})
				end
			end
			...
			table.insert(connections,workspace.DescendantAdded:Connect(optimizeInstance))
			...
			if lplr.Character then
				onCharacter(lplr.Character)
			end

			table.insert(connections,lplr.CharacterAdded:Connect(onCharacter))
			...
			pcall(function()
				UserSettings():GetService("UserGameSettings").SavedQualityLevel = RamSaverSets.SavedQualityLevel
			end)

			pcall(function()
				settings().Rendering.QualityLevel = RamSaverSets.RenderingLevel
			end)

			for _, data in ipairs(RamSaverSets.LightingCachedSets) do
				if #data.ID == 38 then
					if data.SpawnedTime < time() then
						if Lighting:FindFirstChild(data.Name) then
							local obj = Lighting:FindFirstChild(data.Name)
							if obj:IsA("PostEffect") then
								obj.Enabled = data.OG
							end
						elseif Lighting[data.Name] ~= nil then
							Lighting[data.Name] = data.OG
						end
					end
				end
			end
			...
			if Terrain then
				for _, data in ipairs(RamSaverSets.TerrainCachedSets) do
					if #data.ID == 38 then
						if data.SpawnedTime < time() then
							if Terrain[data.Name] ~= nil then
								Terrain[data.Name] = data.OG
							end
						end
					end
				end
			...
			for _, data in ipairs(RamSaverSets.PartsDetailCachedSets) do
				local obj = nil

				for _, inst in ipairs(workspace:GetDescendants()) do
					if inst.Name == data.Name then
						obj = inst
						break
					end
				end
				...
				if obj then
					if #data.ID == 38 then
						if data.SpawnedTime < time() then
							if data.OG3 ~= nil then
								obj.CastShadow = data.OG
								obj.Material = data.OG2
								obj.Reflectance = data.OG3
							elseif data.OG2 ~= nil then
								obj.TextureID = data.OG
								obj.Material = data.OG2
							elseif data.OG ~= nil then
			...
			for _, data in ipairs(RamSaverSets.PartsDetailCachedSets) do
				if typeof(data.OG) == "Instance" then
					if #data.ID == 38 then
						if data.SpawnedTime < time() then
							data.OG.Parent = workspace
						end
					end
				end
			end
    ...
    local function restoreAll()
		...
		cleanupDeadReferences()
		PlaceOrbsOGSlot()
		RamSaverProcess(false)
		table.clear(cachedColors)
		table.clear(originalTextures)
		table.clear(TexturesRemovedProcessedObjects)
		...
		table.clear(RamSaverSets.LightingCachedSets)
		table.clear(RamSaverSets.TerrainCachedSets)
		table.clear(RamSaverSets.PartsDetailCachedSets)
    ...
    FPSBoost = vape.Categories.Legit:CreateModule({
        ...
        Function = function(callback)
            if callback then
                applyAll()
				if OrbRemover.Enabled then
					local Folder = Instance.new('Folder')
					Folder.Parent = replicatedStorage
					Folder.Name = 'OnyxFPSBoostFolder'
				end
                table.insert(connections,
                    workspace.DescendantAdded:Connect(function(obj)
                        if ParticleToggle.Enabled then removeParticle(obj) end
                        if ShadowToggle.Enabled then removeShadow(obj) end
						if OrbRemover.Enabled then RemoveOrbs(obj) end
				...
				if BlocksEffects.Enabled then
					processExistingBlocks(true)
					setupBlockMonitor(true)
				end
				if RamSaver.Enabled then
					RamSaverProcess(true)
				end
    ...
    ParticleToggle = FPSBoost:CreateToggle({
		Name="Particle Remover", 
		Default=true,
        Function=function()
            if FPSBoost.Enabled and ParticleToggle.Enabled then
                FPSBoost:Toggle()
                task.wait()
                FPSBoost:Toggle()
            end
    ...
    ShadowToggle = FPSBoost:CreateToggle({
		Name="Shadow Remover", 
		Default=true,
        Function=function()
            if FPSBoost.Enabled and ShadowToggle.Enabled then
                FPSBoost:Toggle()
                task.wait()
                FPSBoost:Toggle()
            end
    ...
    ArmorToggle = FPSBoost:CreateToggle({
		Name="Armor Remover", 
		Default=false,
        Function=function()
            if FPSBoost.Enabled and ArmorToggle.Enabled then
                FPSBoost:Toggle()
                task.wait()
                FPSBoost:Toggle()
            end
    ...
    TitanToggle = FPSBoost:CreateToggle({
		Name="Titan Remover", 
		Default=true,
        Function=function()
    ...
    EffectsOnlyToggle = FPSBoost:CreateToggle({
        ...
        Default=false,
        Function=function()
            if FPSBoost.Enabled and TitanToggle.Enabled and EffectsOnlyToggle.Enabled then
                FPSBoost:Toggle()
                task.wait()
                FPSBoost:Toggle()
            end
    ...
    TextureRemover = FPSBoost:CreateToggle({
		...
		Default=false,
        Function=function()
            if FPSBoost.Enabled and TextureRemover.Enabled then
                FPSBoost:Toggle()
                task.wait()
                FPSBoost:Toggle()
            end
    ...
    BlocksEffects = FPSBoost:CreateToggle({
		Name="Blocks Effects", 
		Default=false,
        Function=function()
            if FPSBoost.Enabled and BlocksEffects.Enabled then
                FPSBoost:Toggle()
                task.wait()
                FPSBoost:Toggle()
            end
    ...
    RamSaver = FPSBoost:CreateToggle({
		Name="Ram Saver", 
		Default=false,
        Function=function()
            if FPSBoost.Enabled and RamSaver.Enabled then
                FPSBoost:Toggle()
                task.wait()
                FPSBoost:Toggle()
            end
	...
	OrbRemover = FPSBoost:CreateToggle({
		Name="Kit Effect", 
		Default=false,
        Function=function()
            if FPSBoost.Enabled and OrbRemover.Enabled then
                FPSBoost:Toggle()
                task.wait()
                FPSBoost:Toggle()
            end
...
run(function()
		...
		local function attemptBreak(tab, localPosition)
			...
			if BS.Value == "Nearest" and SameBlockProt.Enabled and lockedTarget then
				if isBlockValid(lockedTarget) then
					local dist = (lockedTarget.Position - origin).Magnitude
					if dist < Range.Value and bedwars.BlockController:isBlockBreakable({ blockPosition = lockedTarget.Position / 3 }, lplr) then
						if not SelfBreak.Enabled and lockedTarget:GetAttribute("PlacedByUserId") == lplr.UserId then 
							lockedTarget = nil
							lockedPath = nil
						elseif (lockedTarget:GetAttribute("BedShieldEndTime") or 0) > workspace:GetServerTimeNow() then
							lockedTarget = nil
							lockedPath = nil
						else
							local blockTeam = lockedTarget:GetAttribute("Team") or 0
							local myTeam = lplr.Character:GetAttribute("Team") or -1
							if not SelfBreak.Enabled and blockTeam == myTeam then
								lockedTarget = nil
								lockedPath = nil
							else
								...
								hit += 1
								local target, path, endpos = bedwars.breakBlock(lockedTarget, Effect.Enabled, Animation.Enabled, CustomHealth.Enabled and customHealthbar or nil, AutoTool.Enabled, WallCheck.Enabled, Cache.Enabled)
			...
			for _, v in tab do
				...
				if dist < Range.Value and bedwars.BlockController:isBlockBreakable({ blockPosition = v.Position / 3 }, lplr) then
					...
					if BS.Value == "Nearest" and SameBlockProt.Enabled then
						lockedTarget = v
					end
		...
		BS = Breaker:CreateDropdown({
			...
			Function = function()
				lockedTarget = nil
				lockedPath = nil
				if not SameBlockProt then
					repeat task.wait(0.01) until SameBlockProt
				end
				SameBlockProt.Object.Visible = BS.Value == 'Nearest'
			end
		})
		...
		BM = Breaker:CreateDropdown({
			...
			Default = "Position",
			Visible = true,
			Tooltip = "Position: Break from player position | Mouse: Break from mouse cursor"
		})
		
		SameBlockProt = Breaker:CreateToggle({
			Name = "Same Block",
			Default = false,
			Darker = true,
			Visible = BS.Value == 'Nearest',
			Tooltip = "Locks onto the same block/path until it's destroyed",
			Function = function(v)
				if not v then
					lockedTarget = nil
					lockedPath = nil
...
run(function()	
	...
	NM = vape.Categories.Render:CreateModule({
		...
		Alias = {'NM','Nightmare','Emote','ClientSided','CS'},
...
run(function()
	...
	local function Added(v, icon,non)
		...
		if v.Name:find("crit") or v.Name:lower():find("crit") then
			result = bedwars.getIcon({itemType = 'crit_star'}, true)
		elseif v.Name:find("vitality") or v.Name:lower():find("vitality") then
			result = bedwars.getIcon({itemType = 'vitality_star'}, true)
		else
			result = result
		end
		image.Image = result		
	...
	KitESP = vape.Categories.Kits:CreateModule({
		Name = 'KitESP',
		Alias = {'MetalESP',"StarESP",'ESP','ElderESP','WrenESP'},
		Function = function(callback)
			if callback then
		...
		Function = function(callback)      
			if callback then
				if Bypass.Enabled then
					bedwars.Client:Get(remotes.AFK):SendToServer({afk = false})
					task.wait(0.025)
...
run(function()
    ...
    BetterWhisper = vape.Categories.Kits:CreateModule({
        ...
        Function = function(callback)
			...
			if store.equippedKit ~= "owl" then
				...
				BetterWhisper:Clean(bedwars.Client:Get(remotes.SummonedOWL):Connect(function(plr,target)
					if plr == lplr then
				...
				BetterWhisper:Clean(bedwars.Client:Get(remotes.DeattachedOWL):Connect(function(plr)
					if plr == lplr then
						isWhispering = false
					end
				end))
				task.spawn(function()
					while BetterWhisper.Enabled do
						if not entitylib.isAlive then
							task.wait(0.1)
							continue
						end
						if not AutoMount.Enabled then
							task.wait(0.15)
							continue
						end
	...
	Target = BetterWhisper:CreateDropdown({
		Name = "Target",
		List = getTeammateList(),
		Darker = true,
		Visible = false
	})
	MountRange = BetterWhisper:CreateSlider({
		Name = 'Mount Range',																																																																							
		Min = 0,
		Max = 16,
		Default = 12,
	...
	Limits = BetterWhisper:CreateToggle({
		Name = 'Limt To Owl Orb',
		Default = true,
		Visible = false,
				Darker = true,
	})
	AutoMount = BetterWhisper:CreateToggle({
		Name = 'Auto Owl',
		Default = false,
		Visible = true,
		Function = function(v)
			Limits.Object.Visible = v
			MountRange.Object.Visible = v
			Target.Object.Visible = v
		end
	})
end)
...
run(function()
	...
	BetterMetal = vape.Categories.Kits:CreateModule({
		Name = "AutoMetal",
		Alias = {'Kit'},
		Tooltip = 'makes you play better than bobcat at metal or any1 whos good(js naming sm1 i know who mains metal)',
		Function = function(callback)
						...
						if obj:IsA("Model") and obj.PrimaryPart then
							...
							if distance <= range then
								if StreamerMode.Enabled then
									...
									pcall(function()
										bedwars.Client:Get(remotes.collectEntity):SendToServer({id = obj:GetAttribute("Id")})
									end)
									task.wait(0.1)
								end
							end
		...
		Function = function(callback)
			...
			if callback then
				...
				bedwars.Client.Get = function(self,remote)
					local call = old(self, remote)
					if remote == remotes.PaladinRequest then
						return {
							instance = call.instance,
				...
				BetterLani:Clean(lplr:GetAttributeChangedSignal("PaladinStartTime"):Connect(function()
					...
					if bedwars.AbilityController:canUseAbility('PALADIN_ABILITY') then
						local plr = playersService:WaitForChild(Player.Value)
						if plr.Character and plr then
							bedwars.Client:Get(remotes.PaladinRequest):SendToServer({target = plr})
						else
							bedwars.Client:Get(remotes.PaladinRequest):SendToServer({})
						end	
						task.wait(0.022)
			...
			if callback then
				...
				repeat
					...
					if GUICheck.Enabled then
						shouldBank = bedwars.AppController:isAppOpen('ChestApp') or bedwars.AppController:isAppOpen('BedwarsAppIds.CHEST_INVENTORY')
					else
...
run(function()
	...
	TaxRemover = vape.Categories.Blatant:CreateModule({
		Name = "TaxRemover",
		IsPrem = true,
		Alias = {'ShopTaxDisabler','TaxDisabler'},
		Function = function(callback)
			if callback then
...
run(function()
			...
			if callback then
				...
				if AutoPlay.Enabled then
					...
					bedwars.FishingMinigameController.startMinigame = function(self, Data, results)
						...
						if BlacklistOption.Enabled then
							if table.find(Blacklist.ListEnabled, Data.fishModel) then
								if lplr.Character.Humanoid:GetState() ~= Enum.HumanoidStateType.Jumping then
				...
				if AutoCast.Enabled then
					task.spawn(function()
						repeat
							if entitylib.isAlive then
								local rod = getItem('fishing_rod')
								if rod then
									rayCheck.FilterDescendantsInstances = {lplr.Character, gameCamera}
									local root = entitylib.character.RootPart
									local movedir = root.Position
									...
									if not ray then
										...
										if check then
											if store.hand.tool.Name == 'fishing_rod' then
												launchProjectile(getItem('fishing_rod'), entitylib.isAlive and entitylib.character.RootPart.Position, 'fisherman_bobber', 60, entitylib.character.RootPart.CFrame.LookVector)
											end
										end
									end
								end
	...
	AutoCast = BetterFisher:CreateToggle({
		Name = "AutoCast",
		Default = false,
		Function = function(v)
			if v then
...
run(function()
						...
						if obj then
							...
							if distance <= range then
								if Streamer.Enabled then
									local Key = obj:FindFirstChild('treeOrb').KeyboardKeyCode
									vim:SendKeyEvent(true, Key, false, game)
									task.wait(obj:FindFirstChild('treeOrb').HoldDuration + math.random())
									vim:SendKeyEvent(false, Key, false, game)
								else
									local waitTime = .854 
									task.wait(waitTime)
									...
									pcall(function()
										bedwars.Client:Get(remotes.treeOrb):CallServer({treeOrbSecret = obj:GetAttribute("TreeOrbSecret")})
									end)
									task.wait(0.1)
								end
							end
	...
	Streamer = BetterElder:CreateToggle({Name='Streamer',Default=false})
						...
						if distance <= Range.Value then
							if AimTypes.Value == "Mouse" then
								...
								if AutoLaunch.Enabled then
									...
									local call = bedwars.Client:Get(remotes.CannonLaunch):CallServer({cannonBlockPos = bedwars.BlockController:getBlockPosition(cannon.Position)})
									if call then
            ...
            if callback then
                ...
                local function setupKitRender(obj)
                    if obj.Name == "PlayerRender" and obj.Parent and obj.Parent.Parent and obj.Parent.Parent.Parent and obj.Parent.Parent.Parent.Parent and obj.Parent.Parent.Parent.Parent.Parent and obj.Parent.Parent.Parent.Parent.Parent.Name == "MatchDraftTeamCardRow" then
                        ...
                        updateKit()
                        task.spawn(function()
							while KitRender.Enabled and Rank and Rank.Parent do
...
run(function()
	...
	local function setUpUIAK(kit,sets)
		local ak = AutoKit
		if kit == 'ice_queen' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 30,
				Default = 16,
			...
			sets.Threshold = ak:CreateSlider({
				Name = "Threshold",
				Min = 0,
				Max = 3,
				Default = 0.045,
		...
		elseif kit == 'necromancer' then
			...
			sets.Animations = ak:CreateToggle({
				Name = "Animations",
				Default = true,
				Visible = true,
			})
		elseif kit == 'paladin' then
			...
			sets.TeamOnly = ak:CreateToggle({
				Name = "Team Only",
				Default = true,
				Visible = true,
			})
		elseif kit == 'spearman' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 30,
		...
		elseif kit == 'styx' then
			sets.AutoUseRange = ak:CreateSlider({
				Name = "Auto Use Distance",
				Min = 0,
				Max = 12,
				Default = 8,
				Visible = true
			}) 
			sets.OpenPortalRange = ak:CreateSlider({
				Name = "Open Portal Range",
				Min = 0,
				Max = 16,
				Default = 8,
				Visible = true,
			})
		elseif kit == 'elektra' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 23,
		...
		elseif kit == 'taliyah' or kit == 'farmer_cletus' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 30,
		...
		elseif kit == 'black_market_trader' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 30,
		...
		elseif kit == 'rebellion_leader' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 30,
		...
		elseif kit == 'ninja' or kit == 'frosty' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 30,
		...
		elseif kit == 'sheep_herder' or kit == 'yeti' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 30,
		...
		elseif kit == 'dragon_sword' then
			sets.Range1 = ak:CreateSlider({
				Name = "Range 1",
				Min = 0,
				Max = 30,
				Default = 10,
		...
		elseif kit == 'defender' then
			...
			sets.Sounds = ak:CreateToggle({
				Name = "Sounds",
				Default = true,
				Visible = true
			})

		elseif kit == 'shielder' then
			sets.Rate = ak:CreateSlider({
				Name = "Rate",
				Min = 0,
				Max = 144,
				Default = 60,
		...
		elseif kit == 'sorcerer' or kit == 'grim_reaper' or kit == 'jailor' or kit == 'hannah' or kit == 'warlock' or kit == 'void_dragon' or kit == 'mimic' or kit == 'pinata' or kit == 'block_kicker' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 30,
		...
		elseif kit == 'cactus' or kit == 'card' then
			sets.Range = ak:CreateSlider({
				Name = "Distance",
				Min = 0,
				Max = 30,
		...
		elseif kit == 'drill' then
			sets.Exploit = ak:CreateToggle({
				Name = "Exploit",
				Default = false,
				Visible = true
			})
	...
	local AutoKitFunctions = {
		jellyfish = function(sets)
			local function CheckJellyFish(jelly)
				return jelly:GetAttribute("PlacedByUserId") == lplr.UserId
			end
			repeat
				if not entitylib.isAlive then
					task.wait(0.1)
				...
				for i, v in workspace:GetDescendants() do
					if v.Name == 'Jellyfish' then
						if CheckJellyFish(v) then
							local delta = (v.PrimaryPart.Position - entitylib.character.RootPart.Position).Magnitude
							if delta > (sets.Range.Value + 3 + (math.random() - math.random() * Random.new():NextNumber())) then
								task.wait(0.1)
								continue
							end
						end
		...
		ice_queen = function(sets)
			...
			repeat
				if not entitylib.isAlive then
					...
					if plr then
						local new = tick()
						if (new - last) >= sets.Threshold.Value then
							last = new
							task.wait(0.01) 
							continue
						end	
		...
		necromancer = function(sets)
			local r = sets.Range.Value
			kitCollection('Gravestone', function(v)
				...
				if bedwars.Client:Get(remotes.Gravestone):CallServer({skeletonData={armorType=armorType,weaponType=weaponType,associatedPlayerUserId=associatedPlayerUserId},secret=secret,position=position}) then
					if sets.Animations.Enabled then
						bedwars.GameAnimationUtil:playAnimation(lplr.Character, bedwars.AnimationType.PUNCH)
						bedwars.ViewmodelController:playAnimation(bedwars.AnimationType.FP_USE_ITEM)
						bedwars.SoundManager:playSound(bedwars.SoundList.GRAVESTONE_USE)
					end
				end
			end, sets.Range.Value, false)
		end,
		paladin = function(sets)
			local t = sets.Delay.Value
			local function getLowestHPPlayer()
				local lowestPlayer
				local lowestHP = math.huge
				if sets.TeamOnly.Enabled then
		...
		spearman = function(sets)
			local function fireSpear(pos, spot, item,limits)
				if limits then
					if store.hand.tool.Name ~= 'spear' then
						return
					end
				end
				local projectileRemote = {InvokeServer = function(sets) end}
				task.spawn(function()
					projectileRemote = bedwars.Client:Get(remotes.FireProjectile).instance
				end)
				if item then		
			...
			repeat
				...
				if plr and (lplr.Character:GetAttribute("Health") or 0) > 0 then
					...
					if spearTool then
						fireSpear(pos,spot,spearTool,sets.Limits.Enabled)
					end
		        end
		...
		void_walker = function(sets)
			...
			repeat
				...
				if lplr.Character:GetAttribute('Health') <= 56 then
					local new = tick()
					if (new - last) >= sets.Threshold.Value then
						last = new
						task.wait(0.01) 
		...
		falconer = function(sets)
			...
			repeat
				...
				local plr = entitylib.EntityPosition({
					...
					Players = true,
					Sort = sortmethods[Sorts.Value],
					Wallcheck = sets.Wallcheck.Enabled
				})
		...
		styx = function(sets)
			local r = sets.AutoUseRange.Value
			local uuid  = ""
		...
		elektra = function(sets)
			math.randomseed(os.time() * 9e6)
			local rng = sets.Chance.Value
			local last = 0
			repeat
				if not entitylib.isAlive then task.wait(0.1); continue end
				...
				if plr and (lplr.Character:GetAttribute("Health") or 0) > 0 then
					...
					if bedwars.AbilityController:canUseAbility('ELECTRIC_DASH') and math.random(0,100) >= sets.Chance.Value then
						bedwars.AbilityController:useAbility('ELECTRIC_DASH')
					end																		
		        end
		...
		taliyah = function(sets)
			...
			kitCollection('HarvestableCrop', function(v)
				if bedwars.Client:Get(remotes.Harvest):CallServer({position = bedwars.BlockController:getBlockPosition(v.Position)}) then
					if sets.Animations.Enabled then
						bedwars.GameAnimationUtil:playAnimation(lplr.Character, bedwars.AnimationType.PUNCH)
						local currentsound = 'CHICKEN_ATTACK_1'
						local rng = math.random(1,3)
						currentsound = `CHICKEN_ATTACK_{tostring(rng)}`
						if currentsound == nil then
							currentsound = 'CHICKEN_ATTACK_1'
						end
						bedwars.SoundManager:playSound(bedwars.SoundList[currentsound])
					end
				end
			end, sets.Range.Value, false)
		end,
		black_market_trader = function(sets)
			local r = sets.Range.Value
			kitCollection('shadow_coin', function(v)
			    bedwars.Client:Get(remotes.collectEntity):SendToServer({id = v:GetAttribute("Id"),collectableName = 'shadow_coin'})
			end, sets.Range.Value, false)
		end,
		rebellion_leader = function(sets)
			local last = 0
		...
		ninja = function(sets)
			...
			local function fireUmeko(pos, spot, item,slot,charm,limits)
				if limits then
					if store.hand.tool.Name ~= charm then
						return
					end
				end
				if item then		
			...
			repeat
				...
				if plr and (lplr.Character:GetAttribute("Health") or 0) > 0 then
					fireUmeko(plr.RootPart.Position,plr.RootPart.Velocity,item,charmSlot,charm,sets.Limits.Enabled)
				end

				task.wait(0.025)
			until not AutoKit.Enabled
		end,
		frosty = function(sets)
			local function fireball(pos, spot, item,limits)
				if limits then
					if store.hand.tool.Name ~= 'frosted_snowball' then
						return
					end
				end
				local projectileRemote = {InvokeServer = function(sets) end}
			...
			repeat
				...
				if plr and (lplr.Character:GetAttribute("Health") or 0) > 0 then
					...
					if SnowBallTool then
						fireball(pos,spot,SnowBallTool,sets.Limits.Enabled)
					end
		        end
		...
		sheep_herder = function(sets)
			local r = sets.Range.Value
			kitCollection('sheep', function(v)
				bedwars.SheepHerderKitController:tameSheep(v)
			end, sets.Range.Value, false)
		end,
		yeti = function(sets)
			local function getBedNear()
				...
				for _, v in collectionService:GetTagged("bed") do
					if (localPosition - v.Position).Magnitude < sets.Range.Value then
		...
		dragon_sword = function(sets)
		    repeat
			    if not entitylib.isAlive then task.wait(0.1); continue end
				local plr = entitylib.EntityPosition({
					Range = sets.Range1.Value,
					Part = "RootPart",
					Players = true,
					Sort = sortmethods[Sorts.Value],
					Wallcheck = sets.Wallcheck.Enabled
		...
		defender = function(sets)
		    repeat
			    if not entitylib.isAlive then task.wait(0.1); continue end
				if sets.Limits.Enabled then
				...
				for i, v in workspace:GetChildren() do
					if v:IsA("BasePart") then
						if v.Name == "DefenderSchematicBlock" then
							...
							bedwars.Client:Get(remotes.DefenderPlaceBlock):CallServer({["blockPos"] = BP})
							pcall(function()
								if not sets.Sounds.Enabled then
									return
								end
								local sounds = {
				...
				AutoKit:Clean(workspace.ChildAdded:Connect(function(v)
					if v:IsA("BasePart") then
						if v.Name == "DefenderSchematicBlock" then
							...
							v.Grid.Transparency = 1
							local BP = bedwars.BlockController:getBlockPosition(v.Position)
							bedwars.Client:Get(remotes.DefenderPlaceBlock):SendToServer({["blockPos"] = BP})
							pcall(function()
								if not sets.Sounds.Enabled then
									return
								end
		...
		shielder = function(sets)
			local Distance = sets.Range.Value
			local Rate = (1/sets.Rate.Value)
				...
				if (arrow.Name == "crossbow_arrow" or arrow.Name == "arrow" or arrow.Name == "headhunter_arrow") and arrow:IsA("Model") then
					...
					while root and root.Parent do
						...
						if NewDis <= Distance + (2 + math.random(2,5) - math.random()) then
							...
							bedwars.InfernalShieldController:lowerShield()
						end
						task.wait(0.05)
					end
				end
        ...
        midnight = function(sets)
			local old = nil
			local last = 0
			local Amount = sets.Amount.Value
		...
		sorcerer = function(sets)
			local r =sets.Range.Value
			kitCollection('alchemy_crystal', function(v)
			    bedwars.Client:Get(remotes.collectEntity):SendToServer({id = v:GetAttribute("Id"),collectableName = v.Name})
			end, sets.Range.Value, false)
		end,
		berserker = function(sets)
					...
					for _, obj in pairs(mapCFrames:GetChildren()) do
						if obj:IsA("CFrameValue") and string.match(obj.Name, "_bed") then
							if not string.match(obj.Name, teamid .. "_bed") then
								...
								part.CanCollide = false
								part.Anchored = true
								part.Size =  Vector3.new(sets.SizeX.Value, sets.SizeY.Value, sets.SizeZ.Value)
								part.CFrame = obj.Value
								part.Parent = workspace
								part.Name = "AutoKitRagnarPartWMONYX"
								bedwars.QueryUtil:setQueryIgnored(part, true)
		...
		glacial_skater = function(sets)
		    repeat
			    if not entitylib.isAlive then task.wait(0.1); continue end
				if sets.UseExploit.Enabled then
					bedwars.Client:Get(remotes.Momentum):SendToServer({['momentumValue'] = 9e9})
				else
					bedwars.Client:Get(remotes.Momentum):SendToServer({['momentumValue'] = 100})
				end
		        task.wait(0.1)
		    until not AutoKit.Enabled
		end,
		cactus = function(sets)
		    repeat
			    if not entitylib.isAlive then task.wait(0.1); continue end
				local root = entitylib.character.RootPart
				if not root then continue end
		    ...
		    repeat
				...
				if plr then
					if bedwars.AbilityController:canUseAbility('CARD_THROW') then
						bedwars.Client:Get(remotes.ThrowCards):SendToServer({
		...
		drill = function(sets)
			...
			AutoKit:Clean(workspace.DescendantRemoving:Connect(function(obj)
				if not AutoKit.Enabled then return end
				if obj.Name == "Drill" then
					for i, v in ipairs(drills) do
						if v == obj then
			...
			repeat
				...
				for _, drillObj in ipairs(drills) do
					if not sets.Exploit.Enabled then
						if drillObj:FindFirstChild("RootPart") then
							local drillRoot = drillObj.RootPart
							task.wait(sets.Delay.Value + (math.random() - math.random()))
							if (drillRoot.Position - root.Position).Magnitude <= sets.Range.Value then
								bedwars.Client:Get(remotes.ExtractDrill):SendToServer({
									drill = drillObj
								})
							end
						end
					else
						task.wait(sets.Delay.Value - math.random())
						bedwars.Client:Get(remotes.ExtractDrill):SendToServer({
							drill = drillObj
						})
					end
		...
		airbender = function(sets)
			...
			repeat
				...
				if not root then continue end
					...
					if plr2 and sets.MovingTornado.Enabled then
		...
		nazar = function(sets)
			...
			local lastHitTime = 0
			local hitTimeout = sets.timeout.Value
			local LowHealthThreshold = sets.Health.Value
		...
		hatter = function(sets)
			local Delay = sets.Delay.Value
			repeat
		...
		mage = function(sets)
			local r = 0
			if sets.Exploit.Enabled then
				r = math.huge or (2^1024-1)
			else
				r =sets.Range.Value 
			end
			kitCollection('ElementTome', function(v)
				if sets.Animations.Enabled  then bedwars.GameAnimationUtil:playAnimation(lplr, bedwars.AnimationType.PUNCH); bedwars.ViewmodelController:playAnimation(bedwars.AnimationType.FP_USE_ITEM) end
				bedwars.Client:Get(remotes.MageSelect):SendToServer({secret = v:GetAttribute('TomeSecret')})
				v:Destroy()
				task.wait(0.5)
			end, sets.Exploit.Enabled and math.huge or sets.Range.Value, false)
		end,
		pyro = function(sets)
			local last = 0
			repeat																																																										
				...
				if plr and (lplr.Character:GetAttribute("Health") or 0) > 0 then
					...
					if sets.UseSpeed.Enabled then
					Speed:Toggle(true)
					task.wait(sets.Long.Value - (math.random() * math.random()))
					Speed:Toggle(false)
					end
		...
		battery = function(sets)
			...
			repeat
				if entitylib.isAlive then
					...
					for _, part in workspace:GetPartBoundsInRadius(entitylib.character.RootPart.Position, sets.Range.Value, overlapParams) do
		...
		bigman = function(sets)
			...
			kitCollection('treeOrb', function(v)
				task.wait(sets.Delay.Value)
				if sets.Animations.Enabled then
					if bedwars.Client:Get(remotes.treeOrb):CallServer({treeOrbSecret = v:GetAttribute('TreeOrbSecret')}) then
						bedwars.GameAnimationUtil:playAnimation(lplr, bedwars.AnimationType.PUNCH)
						bedwars.ViewmodelController:playAnimation(bedwars.AnimationType.FP_USE_ITEM)
		...
		block_kicker = function()
			repeat
				if entitylib.isAlive and bedwars.AbilityController then
					...
					local blockCount = 0
					if character then
						blockCount = character:GetAttribute('BlockKickerKit_BlockCount') or lplr:GetAttribute('BlockKickerKit_BlockCount') or 0
					else
		...
		davey = function(sets)
			...
			bedwars.CannonHandController.launchSelf = function(...)
				...
				if block:GetAttribute('PlacedByUserId') == lplr.UserId and (block.Position - entitylib.character.RootPart.Position).Magnitude < 30 then
					if sets.AutoSwitch.Enabled then
						local pickaxe = getPickaxeSlot()
						if hotbarSwitch(pickaxe) or store.hand.tool.Name:lower():find("pickaxe") then
				...
				if (workspace:GetServerTimeNow() - self.lastLaunch) < 0.4 then
					if block:GetAttribute('PlacedByUserId') == lplr.UserId and (block.Position - entitylib.character.RootPart.Position).Magnitude < 30 then
						if sets.AutoSwitch.Enabled then
							local pickaxe = getPickaxeSlot()
							if hotbarSwitch(pickaxe) or store.hand.tool.Name:lower():find("pickaxe") then
								task.wait(sets.BreakDelay.Value)
								task.spawn(bedwars.breakBlock, block, false, nil, true)
								task.spawn(bedwars.breakBlock, block, false, nil, true)
							end
						else
		...
		jailor = function(sets)
			local r = sets.Range.Value
			kitCollection('jailor_soul', function(v)
				bedwars.JailorController:collectEntity(lplr, v, 'JailorSoul')
			end, sets.Range.Value, false)
		end,
		grim_reaper = function(sets)
			local r =  sets.Range.Value
		...
		melody = function(sets)
			...
			repeat
				...
				if ent and getItem('guitar') then
					if sets.Limits.Enabled then
						if store.hand.tool.Name ~= 'guitar' then
							task.wait(0.72)
							continue
						end
					end
		...
		metal_detector = function(sets)
			...
			kitCollection('hidden-metal', function(v)
				...
				if sets.Animations.Enabled  then
					...
					bedwars.GameAnimationUtil:playAnimation(lplr,bedwars.AnimationType.SHOVEL_DIG)
					bedwars.SoundManager:playSound(bedwars.SoundList.SNAP_TRAP_CONSUME_MARK)
					bedwars.Client:Get(remotes.collectEntity):SendToServer({
		...
		summoner = function(sets)
			...
			repeat
				...
				if plr  then
				    ...
				    local root = entitylib.character.RootPart
                    local delta = plr.RootPart.Position - root.Position
                    local localfacing = root.CFrame.LookVector * Vector3.new(1, 0, 1)
                    local angle = math.acos(localfacing:Dot((delta * Vector3.new(1, 0, 1)).Unit))
					if angle >= (math.rad(sets.Angle.Value) / 2) then 
						task.wait(0.55)
						continue
					end
				...
				repeat
					if entitylib.isAlive and bedwars.AbilityController then
						...
						if isAiming and aimingTarget and aimingTarget.RootPart then
							...
							local smooth = sets.Smoothness.Value - (math.random() - math.random() * Random.new():NextNumber())
							local direction = currentLook:Lerp(targetDir, smooth).Unit
		...
		blood_assassin = function(sets)
				local hitPlayers = {} 
				local delay = sets.Delay.Value - (math.random() + math.random())
				AutoKit:Clean(vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
					if not entitylib.isAlive then return end
					...
					if attacker == lplr and victim and victim ~= lplr then
						...
						if not activeContract then
							for _, contract in availableContracts do
								if contract.target == victim then
									local dis = (victim.Character.HumanoidRootPart.Position - entitylib.character.RootPart.Position).Magnitude
									local dist = sets.Range.Value
									if sets.ContractSelection.Value == 'FarRange' then
										if dis >= dist then
											task.wait(delay)
											bedwars.Client:Get(remotes.SelectContract):SendToServer({
												contractId = contract.id
											})
											if sets.Deselect.Enabled then
												task.wait(60 + (math.random() * math.random()))
												bedwars.Client:Get(remotes.SelectContract):SendToServer({})
											end
											break
										end
									elseif sets.ContractSelection.Value == 'FirstHit' then
										if dis <= dist then
											task.wait(delay)
											bedwars.Client:Get(remotes.SelectContract):SendToServer({
												contractId = contract.id
									...
									elseif sets.ContractSelection.Value == 'LowHP' then
										local HP = 32
										repeat
										task.wait(0.2)
										until victim.Character:GetAttribute("Health") <= HP 
										if dis <= dist then
											task.wait(delay)
											bedwars.Client:Get(remotes.SelectContract):SendToServer({
												contractId = contract.id
						...
						if v.Targetable and v.Character and v.Player then
							...
							if distance <= (rng + 2) then
								if collectionService:HasTag(v.Character, "MimicBLockPickPocketPlayer") then
									pcall(function()
										local success = bedwars.Client:Get(remotes.PickPocket):CallServer(v.Player)
									end)
									task.wait(0.85)
								end
							end
		...
		gun_blade = function(sets)
			...
			repeat
				if bedwars.AbilityController:canUseAbility('hand_gun') then
					...
					if plr and rng >= sets.RNG.Value then
		...
		wizard = function(sets)
			...
			repeat
				...
				local itemType = store.hand.tool.Name
				local targetPos = plr.RootPart.Position
				if bedwars.AbilityController:canUseAbility(ability) then
					if roll <= sets.Roll1.Value then
						bedwars.AbilityController:useAbility(ability,newproxy(true),{target = targetPos})
					end
				end
				if itemType == "wizard_staff_2" or itemType == "wizard_staff_3" then
				...
				if itemType == "wizard_staff_3" then
					...
					if plr3 then
						if roll <= sets.Roll3.Value then
							if bedwars.AbilityController:canUseAbility('LIGHTNING_STORM') then
								bedwars.AbilityController:useAbility('LIGHTNING_STORM',newproxy(true),{target = targetPos})
								 roll = math.random(0,100)
							end
						elseif roll <= sets.Roll2.Value then
							if bedwars.AbilityController:canUseAbility("SHOCKWAVE") then
								bedwars.AbilityController:useAbility("SHOCKWAVE",newproxy(true),{target = Vector3.zero})
								 roll = math.random(0,100)
							end
		...
		spirit_summoner = function(sets)
			repeat
				...
				local hasStaff = false
				if sets.Limits.Enabled then
					if store.hand.tool.Name ~= 'spirit_staff' then
						hasStaff = false
					else
						hasStaff = true
					end
	...
	AutoKit = vape.Categories.Kits:CreateModule({
		Name = 'AutoKit',
		Alias = {'Kit','Marina','Freyia','Crypt','Lani','Ares','Trixie','Bekzat','Styx','Elektra','Taliyah','Wren','Nahla','Silas','Umeko','Frosty','Sheep','Sheep Herder','Yeti','Lian','Marcel','Infernal','Inferno','Infernal Shielder','Shielder','Alchemist','Nyx','Death Adder','Adder','Death','Ragnar','Rangar','Krystal','Crystal','Terra','Eldertree','Beekeeper','Cobalt','Pyro','Whim','Umbra','Void Knight','Void Nigga','Void','Ramil','Drill','Martin','Fortuna','Caitlyn','Cait','Arachne','Eldric','Xu\'rot','Xurot','Xu rot','Kaida','Star','Stella','Star Collector Stella','Evelynn','Lucia','Lucía','Miner','Melody','Warden','Hannah','Ginger','Gingerbread Man','Davey','Kaliyah','Farmer','Fisherman','Fisher','Milo','Uma','Zeno','Wizard','Zarah','Zarrah'},
		Function = function(callback)
			if not callback then
				local savedValues = {}
				...
				repeat
					...
					if shop then
						...
						if AutoSell.Enabled then
							...
							if currencyMatch and pr then
								
								while AutoSell.Enabled and AutoTal.Enabled do
...
run(function()
	...
	local order = {
		['Brittle -> Damage -> Range'] = {'power', 'heat', 'range'},
		['Damage -> Brittle -> Range'] = {'heat', 'power', 'range'},
		['Brittle -> Range -> Damage'] = {'power', 'range', 'heat'},
		['Damage -> Range -> Brittle'] = {'heat', 'range', 'power'},
		['Range -> Brittle -> Damage'] = {'range', 'power', 'heat'},
		['Range -> Damage -> Brittle'] = {'range', 'heat', 'power'},
	}

	local function getRemote()
		return bedwars.Client:Get(remotes.FlamethrowerUpgrade).instance
	end
		...
		if store and store.inventory and store.inventory.inventory then
			...
			if ok and inv then
				for _, item in pairs(inv) do
					if item and item.itemType == 'flamethrower' then
						return true
					end
				end
			end
	...
	local function getCurrentLevels()
		if store and store.inventory and store.inventory.inventory then
			...
			if ok and inv then
				for _, item in pairs(inv) do
					if item and item.itemType == 'flamethrower' and item.tool then
	...
	local function doUpgrade(upgradeType)
		...
		if s and type(e) == "table" then
			return true, e
		end
		return false, nil
	end

	local function ProcessUpgrade()
		...
		local targetLevels = {
			range = RangeSlider and RangeSlider.Value or 3,
			heat = HeatSlider and HeatSlider.Value or 3,
			power = PowerSlider and PowerSlider.Value or 3,
		}

		local delay = Delay.Value or 0.15
		...
		repeat
			...
			local allDone = true
			for _, upgradeType in ipairs(seq) do
				if not AutoPyro.Enabled or not AutoUpgrade.Enabled then break end

				local current = levels[upgradeType] or 0
				local target = targetLevels[upgradeType] or 3
				
				if current < target then
					allDone = false
					local success, result = doUpgrade(upgradeType)
					if success and result then
						levels[upgradeType] = result[upgradeType] or current + 1
					end
					task.wait(delay)
				end
			end
	...
	AutoPyro = vape.Categories.Kits:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				if AutoUpgrade.Enabled then
					...
					currentThread = task.spawn(ProcessUpgrade)
				end
				if NoSlow.Enabled then
					...
					bedwars.FlamethrowerController.useFlamethrower = function(arg)
						...
						if bedwars.CooldownController:isOnCooldown(ftu) then
							if bedwars.CooldownController:getRemainingCooldown(ftu) < 0.25 then
								bedwars.CooldownController:onCooldownEnded(ftu, "buffer", function()
		...
		Tooltip = 'Auto upgrades Flamethrower for Pyro kit!'
	})

	Sequence = AutoPyro:CreateDropdown({
		Name = "Upgrade Order",
		List = {
			'Brittle -> Damage -> Range',
			'Damage -> Brittle -> Range',
			'Brittle -> Range -> Damage',
			'Damage -> Range -> Brittle',
			'Range -> Brittle -> Damage',
			'Range -> Damage -> Brittle'
		},
		Default = 'Range -> Damage -> Brittle',
		Darker = true,
		Tooltip = 'Priority order for upgrades'
	})
	...
	RangeSlider = AutoPyro:CreateSlider({
		...
		Suffix = '/3',
		Darker = true,
		Tooltip = 'Maximum Range level (0-3)'
	})
	...
	HeatSlider = AutoPyro:CreateSlider({
		...
		Suffix = '/3',
		Darker = true,
		Tooltip = 'Maximum Heat level (0-3)'
	})
	...
	PowerSlider = AutoPyro:CreateSlider({
		...
		Suffix = '/3',
		Darker = true,
		Tooltip = 'Maximum Power level (0-3)'
	})
	...
	Delay = AutoPyro:CreateSlider({
		...
		Suffix = 's',
		Darker = true,
		Tooltip = 'Delay between upgrade attempts'
	})

	GUICheck = AutoPyro:CreateToggle({
		Name = "Require GUI Open",
		Default = false,
		Darker = true,
		Tooltip = 'Only upgrade when Flamethrower GUI is open'
	})

	Limits = AutoPyro:CreateToggle({
		Name = "Require Holding Item",
		Default = false,
		Darker = true,
		Tooltip = 'Only upgrade when holding flamethrower'
	})
	...
	AutoUpgrade = AutoPyro:CreateToggle({
		...
		Function = function(V)
			...
			Delay.Object.Visible = V
			RangeSlider.Object.Visible = V
			HeatSlider.Object.Visible = V
			PowerSlider.Object.Visible = V
		end,
		Darker = true
	})

	NoSlow = AutoPyro:CreateToggle({
		Name = "No Slow",
		Default = false,
		Darker = true,
		Tooltip = 'Remove flamethrower slowdown'
	})
end)
...
run(function()
    ...
    local function getPredictedPosition()
        ...
        local predictio = root.Position + (horizontalVelocity * predictionTime)
	...
	local function GumDropPlace()
		...
		placedpos = placePos
        local originalSlot = store.inventory.hotbarSlot
		local gumdropSlot = getObjSlot('gumdrop_bounce_pad')
	...
	local function GumDropBreak()
		...
		bedwars.LaunchPadController.attemptLaunch = function(...)
			...
			if (workspace:GetServerTimeNow() - self.lastLaunch) < 0.4 then
				if block:GetAttribute('PlacedByUserId') == lplr.UserId and (block.Position - entitylib.character.RootPart.Position).Magnitude < 30 then
					if AutoSwitch.Enabled then
						if hotbarSwitch(pickaxeslot) or store.hand.tool.Name:lower():find("pickaxe") then
							task.wait(BreakDelay.Value)
							task.wait(0.03)
							local placedBlock = getPlacedBlock(placedpos)
							if placedBlock and placedBlock.Name == "gumdrop_bounce_pad" or block then
								task.spawn(bedwars.breakBlock, placedBlock or block, false, nil, true)
								task.wait(0.1)
								hotbarSwitch(ogslot)
							end
						end
					elseif Limits.Enabled then
						if store.hand.tool.Name:lower():find("pickaxe") then
							task.wait(BreakDelay.Value)
							task.wait(0.03)
							local placedBlock = getPlacedBlock(placedpos)
							if placedBlock and placedBlock.Name == "gumdrop_bounce_pad" or block then
	...
	AutoGinger = vape.Categories.Kits:CreateModule({
		Name = 'AutoGinger',
		...
		Function = function(callback)
			...
			if callback then
				...
				if AutoPlace.Enabled then
					AutoGinger:Clean(runService.RenderStepped:Connect(function()
                        if entitylib.isAlive and tick() - lastPlaceTime > 0.15 then
                            GumDropPlace()
                        end
                    end))
				end
	...
	AutoSwitch = AutoGinger:CreateToggle({
		Name='Auto Switch',
		Default = true,
		Darker = true,
		Visible = false,
	})
	Limits = AutoGinger:CreateToggle({
		Name='Limits to item',
		Darker = true,
		Visible = false,
	})
	PlaceDelay = AutoGinger:CreateSlider({
		Name = "Place Delay",
		Min = 0,
		Max = 2,
		Decimal = 5,
	...
	AutoPlace = AutoGinger:CreateToggle({
		Name='Auto Place',
		Function = function(V)
			PlaceDelay.Object.Visible = V
			Limits.Object.Visible = V
			AutoSwitch.Object.Visible = V
		end
	})
	BreakDelay = AutoGinger:CreateSlider({
		Name = "Break Delay",
		Min = 0,
		Max = 2,
		Decimal = 5,
	...
	AutoBreak = AutoGinger:CreateToggle({
		Name='Auto Break',
		Function = function(V)
			BreakDelay.Object.Visible = V
		end
	})
end)
...
run(function()
	...
	AutoTerra = vape.Categories.Kits:CreateModule({
		...
		Function = function(callback)
			if store.equippedKit ~= "block_kicker" then
				vape:CreateNotification("AutoTerra", "Kit required only!", 8, "warning")
				return
			end
			if callback then
				if NoSlow.Enabled then
					...
					bedwars.BlockKickerKitController.onAbilityUsed = function(arg1, arg2, arg3)
						...
						if arg2 == lplr.Character then
							...
							if v78 then
								if bedwars.EntityUtil:getEntity(arg2) then
									local v79 = lplr.Character
									if v79 ~= nil then
										v79 = v79.PrimaryPart
									end
									if v79 then
										if arg3.ability == bedwars.AbilityId.BLOCK_STOMP then
											arg1:playStompEffect(v78)
										elseif arg3.ability == bedwars.AbilityId.BLOCK_KICK then
											local v81 = gameCamera.CFrame.LookVector
											if bedwars.CameraPerspectiveController:getCameraPerspective() == 1 then
												local v82 = v81.X
												local v83 = v81.Y + 0.35
												local v84 = v81.Z
												v81 = Vector3.new(v82, v83, v84)
											end
											local v85 = arg1:getKickBlockProjectileOriginPosition(lplr.Character.PrimaryPart.Position, v81)
											local v86 = tsa(v78, bkka.BlockKickerKit_CurrentKickCombo)
											arg1:playKickAnimation(v78, v86 == nil and 0 or v86, v85, v81)
											local v88 = RefIDS.generateRefId()
											local v89 = {
												["originPosition"] = v85,
												["direction"] = v81,
												["projectileRefId"] = v88,
												["blockType"] = arg1:launchLocalBlockProjectile(v78, v85, v81, v88)
											}
											bedwars.Client:Get(remotes.KickBlock):SendToServer(v89)
										end
									else
										return nil
									end
					...
					_G.AutoTerraSets.StompThread = task.spawn(function()
						repeat
							task.spawn(function()
								repeat
									task.wait(0.1)
								until bedwars.AbilityController:canUseAbility('BLOCK_STOMP')
								task.wait(0.105)
								bedwars.AbilityController:useAbility('BLOCK_STOMP')
							end)
							task.wait(1.05)
						until not AutoStomp.Enabled or not AutoTerra.Enabled
					end)
				end
					...
					_G.AutoTerraSets.KickThread = task.spawn(function()
						repeat
							...
							if plrs then
								...
								local v85 = bedwars.BlockKickerKitController:getKickBlockProjectileOriginPosition(lplr.Character.PrimaryPart.Position, v81)
								local v86 = tsa(v78, bkka.BlockKickerKit_CurrentKickCombo)
								bedwars.BlockKickerKitController:playKickAnimation(v78, v86 == nil and 0 or v86, v85, v81)
								local v88 = RefIDS.generateRefId()
								local v89 = {
									["originPosition"] = v85,
									["direction"] = plrs.RootPart.CFrame,
									["projectileRefId"] = v88,
									["blockType"] = arg1:launchLocalBlockProjectile(v78, v85, v81, v88)
								}
								repeat
									task.wait(0.1)
								until bedwars.AbilityController:canUseAbility('BLOCK_KICK')
								bedwars.Client:Get(remotes.KickBlock):SendToServer(v89)
								bedwars.AbilityController:useAbility('BLOCK_KICK')
							end
						task.wait(1.05)
						until not AutoKick.Enabled or not AutoTerra.Enabled
					end)
				end
					...
					_G.AutoTerraSets.AutoAimThread = task.spawn(function()
						...
						bedwars.Client.Get = function(self, remoteName)
							...
							if remoteName == remotes.KickBlock then
								return {
									instance = call.instance,
									SendToServer = function(arg1,arg2,arg3,arg4)
										local plrs = entitylib.AllPosition({
											Range = Range.Value,
											Wallcheck = Targets.Walls.Enabled,
											Part = "RootPart",
										...
										if plrs then
											arg2 = plrs.RootPart.CFrame
											arg3 = arg3..`-{tostring(workspace:GetServerTimeNow()-0.045)}`
										end
										return call:SendToServer(arg1,arg2,arg3,arg4)
									end
								}
							end
							return call
	...
	Targets = AutoTerra:CreateTargets({
		Players=true,
		Walls=false,
		NPCs=true
	})
	...
	AutoAim = AutoTerra:CreateToggle({
		Name = "Auto Aim",
		Function = function(v)
			if v then
				if _G.AutoTerraSets.AutoAimThread then
...
run(function()
	...
	AutoNyx = vape.Categories.Kits:CreateModule({
		...
		Alias = {'Kit'},
		Function = function(callback)
			if store.equippedKit ~= "midnight" then
				vape:CreateNotification("AutoNyx", "Kit required only!", 8, "warning")
				return
			end
			if callback then
				repeat
...
run(function()
	...
	local old = nil
	ProjectileExploit = vape.Categories.Exploits:CreateModule({
		Name = "ProjectileExploit",
		Tooltip = 'requires terra kit',
		Alias = {'Exploit','Kit'},
		IsPrem = true,
		Function = function(callback)
			if store.equippedKit ~= "block_kicker" then
				vape:CreateNotification("ProjectileExploit", "Kit required only!", 8, "warning")
				return
			end
			if callback then
				old = bedwars.Client.Get
				bedwars.Client.Get = function(self, remoteName)
					local call = old(self, remoteName)
					if remoteName == remote.KickBlock and ProjectileExploit.Enabled then
						return {
							instance = call.instance,
							SendToServer = function(_, data)
...
run(function()
	...
	local last = 0
	local new = tick()
	local refresh = os.clock()


	AutoNahla = vape.Categories.Kits:CreateModule({
		Name = 'AutoNahla',
		Alias = {'Kit'},
		Function = function(callback)
			if store.equippedKit ~= "oasis" then
				vape:CreateNotification("AutoNahla", "Kit required only!", 8, "warning")
				return
			end
			if callback then
				repeat
					...
					if Limits.Enabled then
						if not store.hand.tool.Name:find("oasis_vessel") then
							task.wait(0.2) 
							continue
						end
					end
					if AutoSwap.Enabled then
						new = tick()
						if (new - last) >= Threshold.Value then
							last = new
							task.wait(0.01) 
							continue
						end

						if os.clock() > refresh then
							refresh = os.clock() + RefreshTick.Value * 3.2
					...
					for i, v in entitylib.List do
						...
						if plr then
							local DeltaPOS = (plr.HumanoidRootPart.Position - entitylib.character.RootPart.Position).Magnitude
							if DeltaPOS <= Range.Value and AutoHeal.Enabled then
					...
					task.wait(1/60)
				until not AutoNahla.Enabled
			else
				last = tick() - 1
				new = tick() + 1
				refresh = os.clock() + (RefreshTick.Value + RefreshTick.Value)
			end
		end
	})
...
run(function()
	...
	local function Place(smart,block)
		task.wait(Delay.Value)
		if smart then
			local pos = entitylib.character.RootPart.Position
			pos = pos - Vector3.new(0, (entitylib.character.HipHeight + (entitylib.character.RootPart.Size.Y / 2)) - 3, 0)
			local rounded = Vector3.new(math.round(pos.X / 3) * 3, math.round(pos.Y / 3) * 3, math.round(pos.Z / 3) * 3)
			bedwars.placeBlock(rounded, block, false)
		else
			local pos = entitylib.character.RootPart.Position
			local rounded = Vector3.new(math.round(pos.X / 3) * 3, math.round(pos.Y / 3) * 3, math.round(pos.Z / 3) * 3)
			bedwars.placeBlock(rounded, block, false)
	...
	AutoCon = vape.Categories.Kits:CreateModule({
		Name = "AutoConqueror",
		Function = function(callback)
			if store.equippedKit ~= "vesta" then
				vape:CreateNotification("AutoConqueror", "Kit required only!", 8, "warning")
				return
			end
			if callback then
				repeat
					if Fire.Enabled and store.hand.tool.Name == 'damage_banner' then
						Place(Smart.Enabled,'damage_banner')
					end
					if KB.Enabled and store.hand.tool.Name == 'defense_banner' then
						Place(Smart.Enabled,'defense_banner')
					end
					if Heal.Enabled and store.hand.tool.Name == 'heal_banner' then
						Place(Smart.Enabled,'heal_banner')
					end
					task.wait(1/60)
				until not AutoCon.Enabled
			else
				task.wait(1/420)
			end
		end
	})
	Fire = AutoCon:CreateToggle({
		Name = "Fire Banner",
		Default = false,
	})
	KB = AutoCon:CreateToggle({
		Name = "Defense Banner",
		Default = false,
	})
	Heal = AutoCon:CreateToggle({
		Name = "Heal Banner",
		Default = true,
	})
	Smart = AutoCon:CreateToggle({
		Name = "Smart Placements",
		Default = true,
	})
	Delay = AutoCon:CreateSlider({
		Name = "Delay",
		Min = 0,
		Max = 2,
		Suffix = 's',
...
run(function()
	...
	local function kitCollection(id, func)
		local objs = type(id) == 'table' and id or collection(id, AutoHannah)
		repeat
			if entitylib.isAlive then
				local localPosition = entitylib.character.RootPart.Position
				for _, v in objs do
					if InfiniteFly.Enabled or not AutoHannah.Enabled then break end
					local part = not v:IsA('Model') and v or v.PrimaryPart
	...
	AutoHannah = vape.Categories.Kits:CreateModule({
		...
		Function = function(callback)
			if store.equippedKit ~= "hannah" then
				vape:CreateNotification("AutoHannah", "Kit required only!", 8, "warning")
				return
			end
			if callback then
				repeat
					...
					if not ent or not ent.RootPart then return end
					if Void.Enabled and isAboveVoid(ent.RootPart.Position) then
						task.wait(1/60)
						continue
					end    
					local root = entitylib.character.RootPart
					...
					local flatFacing = root.CFrame.LookVector * Vector3.new(1, 0, 1)
					local angle = math.acos(math.clamp(flatFacing:Dot(flatDelta), -1, 1))
					local halfAngle  = math.rad(Angle.Value) / 2

					if angle >= halfAngle then
						if not KATarget.Enabled then
							return
						end
					end
					task.spawn(function()
	...
	Void = AutoHannah:CreateToggle({
		Name = 'Void',
		Tooltip = 'wont execute the player if they are in the void',
		Default = true,
	})
	...
	Angle = AutoHannah:CreateSlider({
		Name = "Angle",
		Min = 0,
		Max = 360,
		Default = 120,
	})
	Delay = AutoHannah:CreateSlider({
		Name = "Delay",
		Min = 0,
		Max = 2,
		Suffix = 's',
...
run(function()
	...
	local DelayRelease
	local old = nil
	AutoRelease = vape.Categories.Combat:CreateModule({
		Name = "AutoRelease",
		Tooltip = 'Automatically release ur projectile soruce when at a certain charging percentage',
		Function = function(callback)
			if callback then
				old = bedwars.ProjectileController.calculateImportantLaunchValues
				bedwars.ProjectileController.calculateImportantLaunchValues = function(a, b, c, d, e)
					local currentdds = b.drawDurationSeconds
					if not store.hand.tool.Name:find('bow') then
						currentdds = currentdds + math.random()
					end
					local amount = Percent.Value / 100 * 0.58
					if currentdds >= amount then
						task.spawn(function()
							task.wait(DelayRelease.Value)
							mouse1click()
						end)
					end
	...
	Percent = AutoRelease:CreateSlider({
		Name = "Percent",
		Min = 0,
		Max = 100,
		Default = 100,
	...
	DelayRelease = AutoRelease:CreateSlider({
		Name = "Delay Release",
		Min = 0,
		Max = 2,
		Default = 0.1,
...
run(function()
	...
	ElektraExtender = vape.Categories.Kits:CreateModule({
		...
		IsPrem = true,
		Alias = {'Kit', 'Exploit','Elektra'},
		Function = function(callback)
			if callback then
				old_goal = bedwars.BalanceFile.ELEKTRA.ELECTRIC_DASH_DEPTH_GOAL
				bedwars.BalanceFile.ELEKTRA.ELECTRIC_DASH_DEPTH_GOAL = old_goal + (Range.Value * 4)
			else
				bedwars.BalanceFile.ELEKTRA.ELECTRIC_DASH_DEPTH_GOAL = old_goal
				old_goal = nil
			end
		end
	})
	Range = ElektraExtender:CreateSlider({
		...
		Function = function(val)
			if ElektraExtender.Enabled then
				if old_goal then
					bedwars.BalanceFile.ELEKTRA.ELECTRIC_DASH_DEPTH_GOAL = old_goal + (val * 4)
				end
			end
		end
	})
...
run(function()
	local InfiniteRegent
	local old
	InfiniteRegent = vape.Categories.Exploits:CreateModule({
		Name = "InfiniteRegent",
		IsPrem = true,
...
run(function()
	...
	local Projectiles
	local ProjRange
	local ProjMovementRange
	local ProjShouldJump
	local Melees
	local MelTime
	local MelRange
	local MelDownRange
	...
	AutoDodge = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then
				if Projectiles.Enabled then
					AutoDodge:Clean(workspace.DescendantAdded:Connect(function(arrow)
						...
						if (arrow.Name == "crossbow_arrow" or arrow.Name == "arrow" or arrow.Name == "headhunter_arrow") and arrow:IsA("Model") then
							...
							while root and root.Parent do
								NewDis = (lplr.Character.HumanoidRootPart.Position - root.Position).Magnitude
								if NewDis <= ProjRange.Value then
									if ProjShouldJump.Enabled then
										local humanoid = entitylib.character.Humanoid
										if humanoid:GetState() ~= Enum.HumanoidStateType.Jumping then
											humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
										end
									end
									lplr.Character.Humanoid:MoveTo(Vector3.new(ProjMovementRange.Value,0,0))
								end
								task.wait(0.05)
							end
						end
				...
				if Melees.Enabled then
					repeat
						local plrs = entitylib.AllPosition({
							Range = MelRange.Value,
							Wallcheck = Targets.Walls.Enabled,
							Part = "RootPart",
							Players = Targets.Players.Enabled,
							NPCs = Targets.NPCs.Enabled,
						...
						if plrs then
							local ogpos = lplr.Character.HumanoidRootPart.Position
							lplr.Character.HumanoidRootPart.CFrame = CFrame.new(ogpos + Vector3.new(0, -MelDownRange.Value, 0))
							task.wait(MelUpTime.Value)
							lplr.Character.HumanoidRootPart.CFrame = CFrame.new(ogpos)
						end
						task.wait(MelTime.Value)
					until not AutoDodge.Enabled or not Melees.Enabled
				end
			else
	...
	ProjRange = AutoDodge:CreateSlider({
	...
	ProjMovementRange = AutoDodge:CreateSlider({
	...
	ProjShouldJump = AutoDodge:CreateToggle({
		Name = 'Should Jump?',
		Default = true,
		Darker = true
	})
	Projectiles = AutoDodge:CreateToggle({
		...
		Default = true,
		Function = function(v)
			ProjRange.Object.Visible = v
			ProjMovementRange.Object.Visible = v
			ProjShouldJump.Object.Visible = v
		end
	})
	MelTime = AutoDodge:CreateSlider({
		Name = 'Melee - Time',
		Min = 0,
		Max = 1,
		Decimal = 10,
	...
	MelRange = AutoDodge:CreateSlider({
		Name = 'Melee - Range',
		Min = 0,
		Max = 32,
		Default = 23,
	...
	MelDownRange = AutoDodge:CreateSlider({
		Name = 'Melee - Down Range',
		Min = 0,
		Max = 320,
		Decimal = 10,
		Default = 320,
	...
	MelUpTime = AutoDodge:CreateSlider({
		Name = 'Melee - Up Time',
		Min = 0,
		Max = 2,
		Decimal = 10,
	...
	Melees = AutoDodge:CreateToggle({
		Name = "Melees",
		Default = false,
		Function = function(v)
			MelTime.Object.Visible = v
			MelRange.Object.Visible = v
			MelDownRange.Object.Visible = v
			MelUpTime.Object.Visible = v
		end
	})
...
run(function()
	...
	AutoStella = vape.Categories.Kits:CreateModule({
		Name = "AutoStarCollector",
		Alias = {'Kit'},
		Function = function(callback)
			if callback then
				if store.equippedKit ~= "star_collector" then
					vape:CreateNotification("AutoStarCollector","Kit required only!",8,"warning")
					return
				end
				task.spawn(function()
					while AutoStella.Enabled do
						if not entitylib.isAlive then task.wait(0.1); continue end
						local character = entitylib.character
						if not character or not character.RootPart then task.wait(0.1); continue end
						local localPos = character.RootPart.Position
						local trees = collectionService:GetTagged("stars")
						for _, obj in pairs(trees) do
							if obj then
								local Pos = obj.PrimaryPart.Position
								local distance = (localPos - Pos).Magnitude
								...
								if distance <= range then
									task.wait(Delay.Value)
									if Streamer.Enabled then
										local prompt = obj:FindFirstChild("stars_ProximityPrompt")
										if not prompt then task.wait(0.1); continue end
										local Key = prompt.KeyboardKeyCode
										vim:SendKeyEvent(true, Key, false, game)
										task.wait(obj:FindFirstChild('stars_ProximityPrompt').HoldDuration + math.random())
										vim:SendKeyEvent(false, Key, false, game)
									else
										if Animations.Enabled then
											bedwars.StarCollectorController:collectEntity(lplr, obj, obj.Name)
											if cooldowns[obj] and tick() - cooldowns[obj] < thres then
												continue
											end
											cooldowns[obj] = tick()
											bedwars.GameAnimationUtil:playAnimation(lplr, bedwars.AnimationType.PUNCH)
						...
						if AutoPop.Enabled then
							for i, v in store.inventory.hotbar do
								if v.item then
									if v.item.itemType == 'crit_star' or v.item.itemType == 'vitality_star' then
										task.wait(PopDelay.Value)
										if Limits.Enabled then
											if store.hand.tool.Name == v.item.itemType then
												bedwars.Client:Get(remotes.ConsumeItem):CallServer({
													item = store.hand.tool
												})
											end
										end
										if AS.Enabled then
											if store.hand.tool.Name:lower():find('star') then
												local obj = getObjSlot(store.hand.tool.Name)
												if hotbarSwitch(obj) or store.hand.tool.Name:lower():find('star') then
													bedwars.Client:Get(remotes.ConsumeItem):CallServer({
														item = store.hand.tool
													})
												end
	...
	Animations = AutoStella:CreateToggle({Name='Animations',Default=true})
	Streamer = AutoStella:CreateToggle({Name='Streamer',Default=false})
	AutoPop = AutoStella:CreateToggle({
		Name = 'Auto Pop',
		Tooltip = 'Auto pops each star you have',
		Default = false,
		Function = function(v)
			if PopDelay.Object then
				PopDelay.Object.Visible = v
				AS.Object.Visible = v
				Limits.Object.Visible = v
			end
		end
	})
	PopDelay = AutoStella:CreateSlider({
		Name = "Pop Delay",
		Min = 0,
		Max = 3,
		Default = 1.5,
	...
	AS = AutoStella:CreateToggle({Name='Auto Swap',Default=false,Visible=false,Darker=true})
	Limits = AutoStella:CreateToggle({Name='Limit to items',Default=true,Visible=false,Darker=true})
end)
...
run(function()
	...
	AutoLassy = vape.Categories.Kits:CreateModule({
		Name = "AutoLassy",
		Alias = {'Kit'},
		Tooltip = 'when swinging ur sword it will auto swap to lasso within that millisecond',
		Function = function(callback)
			if store.equippedKit ~= "cowgirl" then
				vape:CreateNotification("AutoLassy","Kit required only!",8,"warning")
				return
			end
			if callback then
				old = bedwars.Client.Get
					...
					if remoteName == remotes.AttackEntity then
						return {
							...
							SendToServer = function(_, attackTable, ...)
								...
								call:SendToServer(attackTable, ...)
								
								if lassoSlot then
									task.delay(MS.Value / 1000, function()
...
run(function()
	...
	local lastChestCheck = 0
	local itemsToDeposit = {}
	local cachedChest = replicatedStorage.Inventories:FindFirstChild(lplr.Name..'_personal')

	local function LootNearestChest(chest)
		chest = chest and chest.Value or nil
		local items = chest and chest:GetChildren() or {}
		if #items > 1 and (Delays[chest] or 0) < tick() then
			bedwars.Client:GetNamespace('Inventory'):Get('SetObservedChest'):SendToServer(chest:FindFirstChild("ChestFolderValue") and chest:FindFirstChild("ChestFolderValue").Value)
			Delays[chest] = tick() + (math.max(Delay.Value,0.15))
			if AnimPlayer.Enabled then pcall(function() bedwars.ChestController.playChestOpenAnimation(chest.Parent) end) end
			bedwars.Client:GetNamespace('Inventory'):Get('SetObservedChest'):SendToServer(chest)
			for _, v in items do
				if v:IsA('Accessory') then
					task.spawn(function()
						pcall(function()
							bedwars.Client:GetNamespace('Inventory'):Get('ChestGetItem'):CallServer(chest, v)
							Chested[tostring(v)] = v
						end)
					end)
				end
				task.wait(math.max(Delay.Value,0.015))
			end
		end
	end

	local function PChestLoot()
		local currentTime = tick()
		
		if not cachedChest then warn('returned') return end
		
		
		for _, v in ipairs(store.inventory.inventory.items) do
			if Chested[v.itemType] then
				table.insert(itemsToDeposit, v)
			end
		end
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					...
					local personal_chests = collection('personal-chest',AutoSteal)
					repeat
						for _, v in chests do
							if v.Name ~= 'personal_chest' then
								local pos = (entitylib.character.RootPart.Position - v.Position).Magnitude
								if pos <= Range.Value then
									if Team.Enabled then
										local ChestTeam = v:GetAttribute('Team') or 0
										local MyTeam = lplr.Character:GetAttribute("Team") or -1
										if ChestTeam == MyTeam then 
											task.wait(0.1)
											continue
									...
									if GUI.Enabled then
										if not bedwars.AppController:isAppOpen('ChestApp') then
											task.wait(0.1)
											continue
										end
									end
									LootNearestChest(v:FindFirstChild('ChestFolderValue'))
									for _, p in personal_chests do
										local pos2 = (entitylib.character.RootPart.Position - p.Position).Magnitude 
										if pos2 <= Range.Value then
											if GUI.Enabled then
												if not bedwars.AppController:isAppOpen('ChestApp') then
													task.wait(0.1)
													continue
	...
	GUI = AutoSteal:CreateToggle({Name='GUI Check',Default = false})
	Team = AutoSteal:CreateToggle({Name='Team Filter',Default = true,Tooltip = 'only apply\'s to the chests'})
	AnimPlayer = AutoSteal:CreateToggle({Name='Animations',Default = false,Tooltip = 'only apply\'s to the chests'})
